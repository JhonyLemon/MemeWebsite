...
public class AdminAccountController{
    ...
    public ResponseEntity<List<AccountPermissionGetDto>> addAccountPermission(@PathVariable Long id, @RequestBody AccountPermissionPostDto accountPermissionPostDto){
        return ResponseEntity.ok().body(accountService.addAccountPermission(id,accountPermissionPostDto));
    }
    @DeleteMapping(path = ApiPaths.Account.ACCOUNT_DELETE_PERMISSION)
    public ResponseEntity<List<AccountPermissionGetDto>> deleteAccountPermission(@PathVariable Long id, @RequestBody AccountPermissionPostDto accountPermissionPostDto){
        return ResponseEntity.ok().body(accountService.deleteAccountPermission(id,accountPermissionPostDto));
    }
